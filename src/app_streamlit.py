...
This is the entry point for the Real-Time Object Detection System.
Run this file with: streamlit run src/app_streamlit.py
"""
...
def initialize_session_state():
    ...
    if 'target_fps' not in st.session_state:
        st.session_state.target_fps = config.DEFAULT_TARGET_FPS
...
def load_detection_engine():
    """Load the YOLOv8 detection engine."""
...
def run_capture_mode():
    ...
    # Show webcam preview using st.camera_input (smooth, no detection)
    ...
    if capture_button:
        ...
        if np_img.shape[1] != config.DEFAULT_FRAME_WIDTH or np_img.shape[0] != config.DEFAULT_FRAME_HEIGHT:
            np_img = cv2.resize(
                np_img,
...
def update_video_frame(frame_placeholder):
    """
    Fragment to update video frame with proper FPS throttling.
    Uses simple timing-based throttling for smooth, lag-free video.
    """
    ...
    target_fps = st.session_state.get('target_fps', config.DEFAULT_TARGET_FPS)
    target_frame_time = 1.0 / target_fps  # Target time per frame in seconds
    
    # Simple, correct FPS throttling using timing
    now = time.time()
    ...
    # Resize frame to smaller resolution for faster YOLO inference
    frame = cv2.resize(
        frame,
        (config.DEFAULT_FRAME_WIDTH, config.DEFAULT_FRAME_HEIGHT),
        interpolation=cv2.INTER_LINEAR
...
def main():
    ...
    **Object Detection using YOLOv8n**
    ...
    This application performs real-time object detection using YOLOv8 nano model.
    **Running on device:** {device_info.upper()}
    ...
    target_fps = st.sidebar.slider(
        "Target FPS (approx)",
        min_value=config.MIN_TARGET_FPS,
        max_value=config.MAX_TARGET_FPS,
        value=min(config.MAX_TARGET_FPS, max(config.MIN_TARGET_FPS, st.session_state.target_fps)),
        step=1,
        help=f"Target frames per second ({config.MIN_TARGET_FPS}-{config.MAX_TARGET_FPS}). Lower values reduce CPU/GPU load."
    )
    ...
    # CRITICAL: Start fragment immediately when running - video appears instantly!
    # Fragment runs independently and updates video frame without rerunning entire script
    if st.session_state.is_running:
        if st.session_state.video_stream and st.session_state.video_stream.is_opened():
            # Fragment starts immediately - video shows FIRST, then detections are added
            # This ensures video appears instantly when Start is clicked
    ...
    **Expected FPS:** 8-15 FPS on typical laptops
    ...
    if st.session_state.video_stream:
        # Note: Streamlit doesn't have a reliable cleanup hook,
        # but resources will be released when the app restarts
        pass


# Streamlit runs the entire script on each interaction, so we call main() at module level
main()
